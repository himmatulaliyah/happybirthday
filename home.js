...
window.addEventListener("load", () => {
  const canvas = document.querySelector(".scratch-container canvas");
  ...
  // Lapisan abu-abu menutupi gambar
  ...
  // === Deteksi seberapa banyak area yang sudah tergosok ===
  function checkReveal() {
    ...
    if (percent > 0.6) { // kalau udah 60% tergosok
      fadeOutMask();
    }
  }
  ...
  setInterval(checkReveal, 1000);
   ...
   // ===== GANTI GAMBAR SAAT DIKLIK (4 FOTO BERURUTAN) =====
const bookImage = document.getElementById('bookImage');
if (bookImage) {
    const images = ['assets/3.jpg', 'assets/4.jpg', 'assets/5.jpg', 'assets/6.jpg'];
    let currentIndex = 0;

    bookImage.addEventListener('click', () => {
        currentIndex = (currentIndex + 1) % images.length; // next index, loop ke 0
